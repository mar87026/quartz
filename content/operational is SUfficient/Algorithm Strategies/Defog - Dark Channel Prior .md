...
t is the medium transmission describing the portion of the light that is **not**
...
But what’s x? Is it the distance to object or the 2D location?  Is A the amplitude?
...
3. The materials we have are 
    1. x, position
    2. I, intensity
    3. Assume the object distance d is unlimited,  $e^{-\beta d(x)}$ is 0. 
...
Now, we undertand the limited value, how about the normal value of t(x)( $e^{-\beta d(x)}$)? 

From first sentense: one of R, G, B should be zero naturally. I(x) = J(x)t(x) + A(1 - t(x))